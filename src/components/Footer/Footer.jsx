import { motion, useInView } from 'framer-motion'
import { useRef } from 'react'
import ScrollReveal from '../ScrollReveal/ScrollReveal'
import styles from './Footer.module.css'

// SVG Icons
const Github = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
  </svg>
)

const LinkedIn = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
  </svg>
)

const Mail = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
    <polyline points="22,6 12,13 2,6" />
  </svg>
)

const Heart = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
    <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
  </svg>
)

const footerLinks = [
  { name: 'Home',     href: '#home'     },
  { name: 'About',    href: '#about'    },
  { name: 'Skills',   href: '#skills'   },
  { name: 'Projects', href: '#projects' },
  { name: 'Contact',  href: '#contact'  },
]

const socialIcons = [
  { icon: <Github />,   url: 'https://github.com/ManzoorAliOfficial',       label: 'GitHub'   },
  { icon: <LinkedIn />, url: 'https://www.linkedin.com/in/manzoorali11',     label: 'LinkedIn' },
  { icon: <Mail />,     url: 'mailto:manzooralidashti11@gmail.com',          label: 'Email'    },
]

function Footer() {
  const currentYear = new Date().getFullYear()
  const bottomRef   = useRef(null)
  const bottomInView = useInView(bottomRef, { once: true, margin: '-40px' })

  return (
    <footer className={styles.footer}>
      <div className="container">

        {/* Main footer content — 3 columns, each with ScrollReveal */}
        <div className={styles.footerContent}>

          {/* Brand — neeche se aayega */}
          <ScrollReveal direction="fromBottom" delay="delay100">
            <div className={styles.footerBrand}>
              <a href="#home" className={styles.logo}>
                Manzoor <span className={styles.logoAccent}>Ali</span>
              </a>
              <p className={styles.tagline}>
                Software Engineering and Frontend Developer.<br />
                Experienced in website cloning and modern web development.<br />
                Specializing in UI/UX design while pursuing a Bachelor's degree (expected 2027).
              </p>
            </div>
          </ScrollReveal>

          {/* Quick Links — thoda delay ke saath */}
          <ScrollReveal direction="fromBottom" delay="delay200">
            <nav className={styles.footerNav}>
              <h3>Quick Links</h3>
              <ul>
                {footerLinks.map((link) => (
                  <li key={link.name}>
                    <a href={link.href}>{link.name}</a>
                  </li>
                ))}
              </ul>
            </nav>
          </ScrollReveal>

          {/* Connect — social icons stagger ke saath */}
          <ScrollReveal direction="fromBottom" delay="delay300">
            <div className={styles.footerConnect}>
              <h3>Connect With Me</h3>
              <div className={styles.socialIcons}>
                {socialIcons.map((item, index) => (
                  <motion.a
                    key={item.label}
                    href={item.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    initial={{ opacity: 0, y: 16 }}
                    animate={bottomInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
                    transition={{ delay: 0.4 + index * 0.1, duration: 0.5 }}
                    whileHover={{ scale: 1.2, y: -4 }}
                    whileTap={{ scale: 0.95 }}
                    aria-label={item.label}
                  >
                    {item.icon}
                  </motion.a>
                ))}
              </div>
            </div>
          </ScrollReveal>

        </div>

        {/* Bottom bar — fadeOnly, slow */}
        <ScrollReveal direction="fadeOnly" speed="slow">
          <div ref={bottomRef} className={styles.footerBottom}>
            <motion.p
              className={styles.copyright}
              initial={{ opacity: 0 }}
              animate={bottomInView ? { opacity: 1 } : { opacity: 0 }}
              transition={{ delay: 0.2, duration: 0.8 }}
            >
              © {currentYear} Mk All rights reserved.
            </motion.p>
            <motion.p
              className={styles.madeWith}
              initial={{ opacity: 0 }}
              animate={bottomInView ? { opacity: 1 } : { opacity: 0 }}
              transition={{ delay: 0.4, duration: 0.8 }}
            >
              Made with <Heart /> passion &amp; purpose
            </motion.p>
          </div>
        </ScrollReveal>

      </div>
    </footer>
  )
}

export default Footer